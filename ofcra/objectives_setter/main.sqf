...
ofcra_redfor_objectives = [];
ofcra_bluefor_objectives = [];
ofcra_redfor_score = 0;
ofcra_bluefor_score = 0;

if (isServer) then {
	// [TYPE, SIDE, PARAMS] call ofcra_fn_create_objective;
	ofcra_fn_create_objective = {

		switch(this select 1) do {
			case "BLUEFOR":	{ 
				[ofcra_bluefor_objectives, this] call BIS_fnc_arrayPush;
			};
			case "REDFOR":	{
				[ofcra_redfor_objectives, this] call BIS_fnc_arrayPush;
			};
			case "BLUEFOR+REDFOR":	{
				[ofcra_bluefor_objectives, this] call BIS_fnc_arrayPush;
				[ofcra_redfor_objectives, this] call BIS_fnc_arrayPush;			
	...
	ofcra_fn_compute_scoreboard = {
...

		{
			...
			if(_side==east) then {
				[ofcra_redfor_survivors, _name] call BIS_fnc_arrayPush;
			}
			else {
				if(_side==west) then { 
					_listId = ;
					[ofcra_bluefor_survivors, _name] call BIS_fnc_arrayPush;
				};
			};
		} forEach allUnits;
		...
		publicVariable ofcra_redfor_score;
		publicVariable ofcra_bluefor_score
